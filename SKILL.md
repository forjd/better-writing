...
1. Read the brief before editing.
   ...
   - For a draft from scratch, collect the substance before writing: the point, the facts, the writer's position, and any example or number they have. If the brief lacks them, ask, or draft with gap markers such as `[your example here]`. An audit can only remove tells that are present. It cannot supply the missing content, and a brief with no content produces generic prose however clean the sentences are.
...
4. Rewrite. Each bullet below is a target illustrated by a preferred versus dispreferred pair; the ban lists in `references/` are for the audit, not for the rewrite.
   ...
   - Rewrite, don't patch. When a sentence carries two or more tells, write it again from what it means. Swapping the flagged word for a synonym leaves the sentence's rhythm and shape, and those are what readers notice. Single-tell sentences and light edits keep the smallest fix.
...
- If the user asks for text that passes an AI detector, run the strict pass and say that no edit guarantees a score. Do not use evasion tricks: synonym swaps, inserted typos, look-alike or zero-width characters, or translation round-trips. They make the prose worse and current detectors are trained on them.
