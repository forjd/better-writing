...
- Humaniser residue. Commercial "humanisers" swap words for dictionary synonyms and damage the text: "counterfeit consciousness" for "artificial intelligence", tortured phrases in place of standard terms, non-standard Unicode look-alike characters, and a sudden drop to elementary register with random typos. A benchmark of 19 such tools found every one degraded the original. Zero-width characters (U+200B, U+200C, U+200D, U+2060, U+FEFF) scattered through plain English words belong here too; they are inserted to break detector tokenisers. Some uses are correct: the joiner (U+200D) inside emoji sequences, joiners and non-joiners (U+200D, U+200C) in scripts that need them such as Persian and the Indic scripts, and U+FEFF as a byte-order mark at the very start of a file. Treat the rest of the residue as conclusive: delete stray zero-width characters and restore the plain term.
...
- anything under about 200 words. Stylometric features, and this skill's judgement, lose most of their accuracy at that length, so a short message gets no stylometric verdict at all; only near-conclusive artefacts count. One commercial classifier trained on large corpora holds up on short text (see below), which is no reason for an editor to judge it

Why single features are unreliable, and why detector-evasion is a non-goal:

- Detectors vary enormously and the bias is documented. A 2023 Stanford study found more than half of non-native English essays misclassified as AI across seven detectors, on a sample of 179 essays. A 2026 ACL study of 16 detectors on about 41,700 essays confirmed the direction: essays by English language learners are more likely to be classified as machine-generated, and non-white learners more so than white ones. Human raters in the same study were no better than chance and showed no such bias. At the other end, two independent studies found one trained commercial classifier near error-free, with the others missing more AI text: an NBER working paper by Jabarian and Imas (September 2025, read directly) and a June 2026 study in the International Journal for Educational Integrity (second-hand). In the NBER study that classifier stayed robust even on passages of 50 words or fewer. OpenAI retired its own classifier in 2023 after it correctly flagged only 26% of AI text. Light paraphrasing still defeats most detectors, though the best ones are now trained on humaniser output.
- Readers split the same way. Lay readers perform near chance and rely on wrong cues, such as treating first person as proof of a human writer (Jakesch et al., PNAS 2023). People who use LLMs for their own writing are strong detectors: in an ACL 2025 study, a majority vote of five such readers misclassified 1 of 300 articles, including paraphrased and humanised ones. Their cues were vocabulary, formality, originality, and clarity, so cutting the tell lists alone is not enough. Generic content with no position reads as machine-made to them even after every tell is gone; see Flatness is also a tell in `voice-and-context.md`.
